...
int main(int argc, char *argv[]) { /* naive_rsend_recv.c */
...
MPI_Status estado;
    ...
    /* Barreira garante que todos terminaram de computar antes de qualquer envio.
       Após a barreira, processo 0 posta MPI_Recv(ANY_SOURCE) e workers chamam
       MPI_Rsend — a proximidade temporal permite o matching correto. */
    MPI_Barrier(MPI_COMM_WORLD);

    if (meu_ranque == 0) {
        total = cont;
        for (origem = 1; origem < num_procs; origem++) {
            MPI_Recv(&cont, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &estado);
            total += cont;
        }
